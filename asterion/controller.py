...
from panda3d.core import KeyboardButton, Quat, Vec3, WindowProperties, lookAt
...
class PlayerController(DirectObject):
    ...
    def __init__(self, app):
        ...
        # Physical attitude survives tangent-frame changes. camera_roll remains
        # the optional, transient presentation bank, independently of this.
        ...
        self._flight_collision = None
        self._space_blend = None
    ...
    def set_space_blend(self, value_or_none):
        """Select continuous atmospheric flight tuning without resetting motion.

        Zero is surface flight and one is space flight. None keeps the legacy
        mode defaults for callers without a spherical flight environment.
        """
        self._space_blend = (None if value_or_none is None else
                             _number(value_or_none, 0.0, 0.0, 1.0))

    def set_flight_collision(self, callback_or_none):
        """Use one sphere sweep for all flight terrain and solid contacts.

        The callback receives (position, displacement, radius) in the active
        frame and returns a collision.MoveResult. Walking keeps capsule and
        height-field collision. Swapping the adapter preserves flight state.
        """
        if callback_or_none is not None and not callable(callback_or_none):
            raise TypeError("Flight collision must be callable or None")
        self._flight_collision = callback_or_none

    def _attitude(self, include_camera_roll=False):
        attitude = Quat()
        roll = self.reference_roll + (self.camera_roll if include_camera_roll else 0.0)
        attitude.setHpr((_number(self.heading), _number(self.pitch), _number(roll)))
        return attitude

    def reframe(self, old_frame, new_frame):
        """Express the same physical pose and velocity in a new rigid frame.

        Both forward and up are transformed, so crossing a pole or changing
        hemispheres cannot silently level the ship. Held controls, banking,
        throttle, FOV, and contact feedback are untouched.
        """
        if old_frame is new_frame:
            return
        from .planetary import (frame_to_local, frame_to_world,
                                vector_to_local, vector_to_world)

        def point(value):
            return frame_to_local(new_frame, frame_to_world(old_frame, value))

        def vector(value):
            return vector_to_local(new_frame, vector_to_world(old_frame, value))

        attitude = self._attitude(include_camera_roll=True)
        local_attitude = Quat()
        lookAt(local_attitude, vector(attitude.getForward()), vector(attitude.getUp()))
        heading, pitch, roll = local_attitude.getHpr()
        self.position = point(self.position)
        self._last_valid_position = point(self._last_valid_position)
        self.velocity = vector(self.velocity)
        self.camera_offset = vector(self.camera_offset)
        self.heading = float(heading) % 360.0
        self.pitch = float(pitch)
        self.reference_roll = float(roll) - self.camera_roll
        self.vertical_speed = float(self.velocity.z)
        self._apply_camera()

    def _flight_parameters(self):
        blend = (float(self.mode == "orbit") if self._space_blend is None
                 else self._space_blend)
        return (35.0 + 75.0 * blend, 100.0 + 700.0 * blend,
                3.2 - 1.3 * blend, 3.0 - 1.5 * blend,
                0.16 + 0.06 * blend)
    ...
    def set_mode(self, mode, position, heading=0, pitch=0):
        ...
        pitch_limit = 85.0 if mode == "surface" else 90.0
        self.pitch = _number(pitch, 0, -pitch_limit, pitch_limit)
        self.speed = 0.0
    ...
    def forward(self):
        return self._attitude().getForward()
    ...
    def _turn_view(self, turn, tilt):
        if self.mode != "surface":
            if turn == 0 and tilt == 0:
                return
            # Ship-local steering has no tangent-frame pole. Compose the
            # physical attitude, keeping optional camera bank out of input.
            delta = Quat()
            delta.setHpr((turn, tilt, 0.0))
            attitude = delta * self._attitude()
            attitude.normalize()
            heading, pitch, roll = attitude.getHpr()
            self.heading = float(heading) % 360.0
            self.pitch = float(pitch)
            self.reference_roll = float(roll)
            return
        self.heading = (_number(self.heading) + turn) % 360.0
        self.pitch = _number(_number(self.pitch) + tilt, 0, -85.0, 85.0)
    ...
    def _fly(self, dt, height_fn, vitals, upgrades):
        ...
        wish = attitude.getForward() * (thrust if thrust >= 0 else thrust * 0.45)
        wish += attitude.getRight() * self._axis("d", "a") * 0.72
        if self._space_blend is None and self._flight_collision is None:
            # Existing flat-world callers retain vertical lift controls.
            up = Vec3(0, 0, 1)
        else:
            up = attitude.getUp()
        wish += up * self._axis("space", "control") * 0.70
        if wish.lengthSquared() > 1:
            wish.normalize()
        ...
        cruise, boost_speed, response, damping, boost_drain = self._flight_parameters()
        cruise *= 1.0 + 0.08 * engine
        maximum = boost_speed * (1.0 + 0.06 * engine) if boost else cruise
        drain = boost_drain if boost else 0.025
        ...
        start = Vec3(self.position)
        if self._flight_collision is not None:
            result = self._flight_collision(start, movement, SHIP_RADIUS)
            self.position = Vec3(result.position)
            self._contacts(result)
            self.grounded = bool(result.grounded)
            return
    ...
    def _update_camera_motion(self, dt, previous, turn_rate):
        ...
        if self.mode == "surface":
            ...
            roll_target = _number(-self._axis("d", "a") * 6.0 - turn_rate * 0.05,
                                  0, -14, 14)
            cruise, maximum, _, _, _ = self._flight_parameters()
    ...
    def _apply_camera(self):
        ...
        if camera is not None:
            ...
            camera.setHpr(self.heading, self.pitch, self.reference_roll + self.camera_roll)
