...
def _system(index):
    ...
    for p in range(PLANETS_PER_SYSTEM):
        ...
        planets.append(dict(
            ...
            position=(round(math.sin(angle) * distance, 3) * SPACE_SCALE,
                      round(math.cos(angle) * distance, 3) * SPACE_SCALE,
                      round(rng.uniform(-320, 420), 3) * SPACE_SCALE),
            size=round(prng.uniform(190, 320), 1) * SPACE_SCALE,
    ...
    return dict(id=index, name=_SYSTEM_NAMES[index], star_color=_STAR_COLORS[index % len(_STAR_COLORS)],
                planets=planets, station=(0.0, 1150.0 * SPACE_SCALE, 170.0 * SPACE_SCALE),
