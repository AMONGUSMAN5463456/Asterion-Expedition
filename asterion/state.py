...
from .universe import SPACE_SCALE, generate_system, get_planet, planet_from_id, terrain_height

SAVE_VERSION = 5
COORDINATE_VERSION = 2
_LEGACY_SPACE_SCALE = SPACE_SCALE
_MAX_SAVED_SPEED = 10000.0
_COORDINATE_LIMIT = 40_000_000.0
...
def _position(value, fallback):
    ...
    return [_number(value[i], float(fallback[i]), -_COORDINATE_LIMIT, _COORDINATE_LIMIT) for i in range(3)]
...
def _valid_position(value):
    ...
    return all(isinstance(x, (float, int)) and not isinstance(x, bool)
               and -_COORDINATE_LIMIT <= x <= _COORDINATE_LIMIT and math.isfinite(x) for x in value)


def _frame_normal(value, fallback):
    """A frame is unanchored, or has a finite, nonzero unit up vector."""
    if value is None:
        return None
    default = None if fallback is None else list(fallback)
    if not isinstance(value, (list, tuple)) or len(value) != 3:
        return default
    ...
    if not all(math.isfinite(x) for x in values):
        return default
    # Scale first so even very large or subnormal finite vectors normalize
    # without overflowing or underflowing their squared length.
...
def _velocity(value):
    if not isinstance(value, (list, tuple)) or len(value) != 3:
        return [0.0, 0.0, 0.0]
    velocity = [_number(x, 0, -_MAX_SAVED_SPEED, _MAX_SAVED_SPEED) for x in value]
    speed = math.hypot(*velocity)
    if speed > _MAX_SAVED_SPEED:
        velocity = [x * (_MAX_SAVED_SPEED / speed) for x in velocity]
    return velocity
...
class GameState:
    def __init__(self, seed=73129):
        ...
        self.coordinate_version = COORDINATE_VERSION
        self.world_scale = SPACE_SCALE
        self.frame_up = [0.0, 0.0, 1.0]
    ...
    def to_dict(self):
        ...
        raw = {key: getattr(self, key) for key in (
            ...
            "coordinate_version", "world_scale", "frame_up", "velocity", "reference_roll",
    ...
    def _from_dict(cls, data):
        ...
        coordinate_version = data.get("coordinate_version")
        legacy_coordinates = (
            ("coordinate_version" not in data and _int(data.get("version"), 1, 1, SAVE_VERSION) < 4)
            or (type(coordinate_version) is int and coordinate_version == 1)
        )
        default_frame = None if state.mode == "orbit" else [0.0, 0.0, 1.0]
        if legacy_coordinates:
            # Versions 1-3 used miniature system coordinates in orbit. Parked
            # ship and base locations are home-surface coordinates and retain
            # their metre scale. Stamp v2 immediately to make migration one-way.
            if state.mode == "orbit":
                state.position = _position([x * _LEGACY_SPACE_SCALE for x in state.position], state.position)
            state.frame_up = default_frame
        else:
            state.frame_up = _frame_normal(data.get("frame_up", default_frame), default_frame)
            state.velocity = _velocity(data.get("velocity"))
            state.reference_roll = (_number(data.get("reference_roll"), 0, -1e7, 1e7) + 180) % 360 - 180
        state.heading = _number(data.get("heading"), 0, -1e7, 1e7) % 360
        # A tangent-frame change can put a flying ship exactly on either pitch
        # pole. Preserve that attitude through disk saves and controller restore.
        pitch_limit = 90 if state.mode in ("flight", "orbit") else 89
        state.pitch = _number(data.get("pitch"), 0, -pitch_limit, pitch_limit)
        ...
        current_format = _int(data.get("version"), 0, 0, SAVE_VERSION) >= 5
        old_scale = _number(data.get("world_scale"),
                            SPACE_SCALE if legacy_coordinates or current_format else 24.0,
                            1.0, SPACE_SCALE)
        if old_scale != SPACE_SCALE and not legacy_coordinates:
            state._resize_world(old_scale)
        return state, changed

    def _resize_world(self, old_scale):
        """Migrate v1.2 poses once without scaling ships, momentum or home bases.

        The frame's tangent offsets stay in metres and its radial datum altitude
        is retained. Deep-space positions follow the expanded chart; positions
        near a planet or station retain their physical clearance from that body.
        This uses doubles and no engine import so saves remain engine-independent.
        """
        ratio = SPACE_SCALE / old_scale
        planet = get_planet(self.system_id, self.planet_index)
        old_radius, new_radius = planet["size"] / ratio, planet["size"]

        def basis(up):
            x, y, z = up
            horizontal = math.hypot(x, z)
            right = (z / horizontal, 0., -x / horizontal) if horizontal > 1e-10 else (1., 0., 0.)
            north = (y * right[2], z * right[0] - x * right[2], -y * right[0])
            return right, north, up

        def resized_local(position):
            x, y, z = position
            datum = math.hypot(x, y, old_radius + z) - old_radius
            axial = math.copysign(math.sqrt(max(0., (new_radius + datum) ** 2 - x*x - y*y)),
                                  old_radius + z)
            return [x, y, axial - new_radius]

        old_position = list(self.position)
        if self.frame_up is not None:
            self.position = resized_local(self.position)
            self.ship_position = resized_local(self.ship_position)
        else:
            system = generate_system(self.system_id)
            nearby = []
            for body in system["planets"]:
                relative = [old_position[i] - body["position"][i] / ratio for i in range(3)]
                radius = body["size"] / ratio
                distance = math.hypot(*relative)
                nearby.append((distance - radius, body, relative, distance))
            clearance, body, relative, distance = min(nearby, key=lambda entry: entry[0])
            if clearance < 5000 and distance > 1:
                self.position = [body["position"][i] + relative[i] / distance *
                                 (body["size"] + clearance) for i in range(3)]
            else:
                station_offset = [old_position[i] - system["station"][i] / ratio for i in range(3)]
                if math.hypot(*station_offset) < 1000:
                    self.position = [system["station"][i] + station_offset[i] for i in range(3)]
                else:
                    self.position = [component * ratio for component in old_position]

        axes = basis(self.frame_up) if self.frame_up else None
        for planet_id, records in self.bases.items():
            body = planet_from_id(planet_id)
            before_radius = body["size"] / ratio
            for record in records:
                u, v, height = record["pos"]
                # The authored home district and its metre addresses stay put.
                if math.hypot(u, v) < 512:
                    continue
                longitude, latitude = u / before_radius, v / before_radius
                direction = (math.sin(longitude) * math.cos(latitude), math.sin(latitude),
                             math.cos(longitude) * math.cos(latitude))
                if axes and planet_id == planet["id"]:
                    relative = [component * (old_radius + height) for component in direction]
                    local = [sum(relative[i] * axis[i] for i in range(3)) for axis in axes]
                    local[2] -= old_radius
                    # Keep nearby construction within walking reach of its save.
                    if math.dist(local, old_position) < 2048 and old_radius + local[2] > 0:
                        local = resized_local(local)
                        relative = [sum(axes[j][i] * local[j] for j in range(3)) +
                                    self.frame_up[i] * new_radius for i in range(3)]
                        length = math.hypot(*relative)
                        direction = [component / length for component in relative]
                        u = math.atan2(direction[0], direction[2]) * new_radius
                        v = math.asin(max(-1., min(1., direction[1]))) * new_radius
                        record["pos"] = [u, v, height]
                        continue
                record["pos"] = [u * ratio, v * ratio, height]
        ...
        try:
            ...
            if path.exists():
                ...
                try:
                    ...
                    pass  # An invalid primary must never replace a good backup.
                if previous is not None:
                    archive = path.with_name(path.name + ".pre-v1.3.bak")
                    if previous.get("version", 1) < 5 and not archive.exists():
                        # Keep the original expedition beyond rolling autosave
                        # backups. This archive is never replaced by this build.
                        with tempfile.NamedTemporaryFile("wb", dir=path.parent,
                                prefix=path.name + ".archive-", delete=False) as stream:
                            backup_temporary = Path(stream.name)
                            stream.write(path.read_bytes())
                            stream.flush()
                            os.fsync(stream.fileno())
                        os.replace(backup_temporary, archive)
                        backup_temporary = None
                    try:
                        ...
                        pass  # Optional rolling backup; original archive is safe.
