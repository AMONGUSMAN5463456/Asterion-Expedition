...
from .atmosphere_effects import AtmosphereEffects
...
class ExpeditionApp(ShowBase):
    ...
    def __init__(self, save_dir=None, offscreen=False, no_audio=False):
        ...
        self.camLens.setNearFar(0.12, 1000000)
        self.game = GameState()
    ...
    def _load_surface(self, fresh=False, landing=None):
        """Set up an explicit new, loaded, or rescued expedition on the ground.

        Flight never calls this method at an atmosphere boundary. The same
        solar scene and geographic terrain remain present throughout a flight.
        """
        self.system = generate_system(self.game.system_id)
        self.planet = self.system["planets"][self.game.planet_index]
        self._remove_ship()
        if self.world.state is not self.game:
            self.world.destroy()
        self.world.ensure_system(self.system, self.game)
        normal = (0, 0, 1) if fresh else (getattr(self.game, "frame_up", None) or (0, 0, 1))
        self.frame = PlanetFrame.from_planet(self.planet, normal)
        if fresh:
            self.game.position = [0, -16, 21.8]
        self.world.prepare_surface(self.planet, self.frame, self.game)
        self.controller.set_collision_world(self.world.collisions)
        self.controller.set_flight_collision(self.world.move_sphere)
        ...
        self.controller.reference_roll = 0 if fresh else getattr(self.game, "reference_roll", 0)
        self.mine_time = 0
        self.target = None
        self.navigation = None
    ...
    def launch(self):
        ...
        self._update_flight_environment()
        self.toast("Launch complete. W thrust | Mouse steer | Space climb | Fly through the atmosphere to space.", "launch", 8)

    def world_position(self):
        return frame_to_world(self.frame, self.controller.position)

    def _restore_flight(self):
        """Restore the saved physical pose, including an in-atmosphere flight."""
        self._remove_ship()
        self.system = generate_system(self.game.system_id)
        self.planet = self.system["planets"][self.game.planet_index]
        if self.world.state is not self.game:
            self.world.destroy()
        self.world.ensure_system(self.system, self.game)
        up = getattr(self.game, "frame_up", None)
        self.frame = PlanetFrame.from_planet(self.planet, up) if up else None
        if self.frame:
            self.world.prepare_surface(self.planet, self.frame, self.game)
        else:
        ...
        self.controller.set_collision_world(self.world.collisions)
        self.controller.set_flight_collision(self.world.move_sphere)
        self.controller.set_mode(self.game.mode, self.game.position,
                                 self.game.heading, self.game.pitch)
        self.controller.velocity = Vec3(*getattr(self.game, "velocity", (0, 0, 0)))
        self.controller.speed = self.controller.velocity.length()
        self.controller.reference_roll = getattr(self.game, "reference_roll", 0)
        self.camera.setHpr(self.controller.heading, self.controller.pitch,
                           self.controller.reference_roll)
        self.target = self.navigation = None
        self._update_flight_environment()
        self.world.update(0, self.controller.position, self.game.elapsed, 0)

    def enter_orbit(self, position=None, restoring=False):
        """Explicit initial placement after an interstellar fold or diagnostics.

        Atmospheric ascent and descent use _update_flight_environment instead.
        No atmosphere shell is a solid, and no altitude calls this initializer.
        """
        if restoring:
        ...
        self.world.set_frame(None)
        if position is None:
            position = Vec3(*self.planet["position"]) + Vec3(0, -self.planet["size"] * 3.2, 0)
        self.controller.set_collision_world(self.world.collisions)
        self.controller.set_flight_collision(self.world.move_sphere)
        self.controller.set_mode("orbit", position, 0, 0)
        self.game.mode = "orbit"
        self.autopilot = self.target = self.navigation = None
        self._update_flight_environment()
        self.world.update(0, self.controller.position, self.game.elapsed, 0)
        self.toast(f"{self.system['name']} // Fly directly into a planet's atmosphere. M plots an approach.", "discover", 8)

    def _change_frame(self, new_frame, planet=None):
        """Change coordinate representation while preserving the physical pose."""
        old = self.frame
        ship_world = frame_to_world(old, self.ship.getPos()) if self.ship else None
        ship_forward = vector_to_world(old, self.ship.getQuat().getForward()) if self.ship else None
        ship_up = vector_to_world(old, self.ship.getQuat().getUp()) if self.ship else None
        self.controller.reframe(old, new_frame)
        self.frame = new_frame
        self.game.position = list(self.controller.position)
        if new_frame:
            self.planet = planet or next(p for p in self.system["planets"] if p["id"] == new_frame.planet_id)
            self.game.planet_index = self.planet["index"]
            self.world.prepare_surface(self.planet, new_frame, self.game,
                                       position=self.controller.position)
        else:
            self.world.set_frame(None)
        if self.ship is not None:
            pos = frame_to_local(new_frame, ship_world)
            self.ship.setPos(pos)
            self.ship.lookAt(pos + vector_to_local(new_frame, ship_forward),
                             vector_to_local(new_frame, ship_up))
            self.game.ship_position = list(pos)
            self._register_ship_collision(pos.x, pos.y, pos.z, self.ship.getH())
        if self.navigation and "world_pos" in self.navigation:
            self.navigation["pos"] = list(frame_to_local(new_frame, self.navigation["world_pos"]))

    def _update_flight_environment(self):
        if not self.world.fields:
            return
        point = self.world_position()
        velocity = vector_to_world(self.frame, self.controller.velocity)
        field = min(self.world.fields.values(), key=lambda f: f.altitude(point))
        data = field.atmosphere(point, velocity)
        altitude = data["altitude"]
        if self.controller.mode == "surface":
            # Walking and jetpack use a tangent frame that follows the globe.
            field = self.world.fields[self.planet["id"]]
            data = field.atmosphere(point, velocity)
            if self.frame and math.hypot(self.controller.position.x, self.controller.position.y) > 450:
                self._change_frame(PlanetFrame.from_planet(self.planet, point - field.center), self.planet)
        else:
            if altitude < 2800:
                planet = field.planet
                if (self.frame is None or self.frame.planet_id != planet["id"] or
                        math.hypot(self.controller.position.x, self.controller.position.y) > 450):
                    self._change_frame(PlanetFrame.from_planet(planet, point - field.center), planet)
                self.planet = planet
                self.game.planet_index = planet["index"]
                if altitude < 180:
                    self.game.visit(planet)
            elif self.frame is not None and altitude > 3200:
                self._change_frame(None)
            # A flight phase is a HUD label, never a controller reset.
            self.controller.mode = "orbit" if data["space_blend"] >= .99999 else "flight"
        self.atmosphere_data = data
        self.controller.set_space_blend(data["space_blend"])
        self.world.mode = "surface" if self.controller.mode != "orbit" else "orbit"
        self.world.set_flight_environment(data)
        self.game.mode = self.controller.mode

    def _geo_position(self, local):
        field = self.world.fields[self.planet["id"]]
        delta = frame_to_world(self.frame, local) - field.center
        u, v = direction_chart(delta, field.radius)
        return [u, v, delta.length() - field.radius]

    def _geo_to_local(self, geo):
        field = self.world.fields[self.planet["id"]]
        point = field.center + chart_direction(geo[0], geo[1], field.radius) * (field.radius + geo[2])
        return frame_to_local(self.frame, point)

    def land_on_planet(self, planet_index):
        """Plot a physical approach; kept as the public navigation entry point."""
        if self.controller.mode == "surface":
        ...
        return self._plan_approach(destination)

    def _touch_down(self):
        p = self.controller.position
        landing = self._find_landing_site(p.x, p.y)
        if landing is None or not self._park_ship(*landing, self.controller.heading):
    ...
    def flight_action(self):
        ...
        if self.controller.mode == "surface":
            self.launch()
        elif self.atmosphere_data["altitude"] <= 65 and self.controller.velocity.length() < 48:
            self._touch_down()
        else:
            self.toast("Fly directly through the atmosphere. F lands below 65 m and 48 m/s. S brakes; Ctrl descends.")
    ...
    def _survival(self, dt):
        ...
        for building in self.game.bases.get(self.planet["id"], []):
            if distance(self.controller.position, self._geo_to_local(building["pos"])) < 22:
    ...
    def _plan_approach(self, destination):
        ...
        start = self.world_position()
        obstacles = [{"id": p["id"], "center": p["position"], "radius": p["size"] + 450}
                     for p in self.system["planets"]]
        obstacles += [{"id": e["id"], "center": frame_to_world(self.frame, e["pos"]),
                       "radius": e.get("radius", 20) + 35}
                      for e in self.world.interactables() if e["kind"] == "asteroid"]
        obstacles.append({"id": "exchange", "center": self.system["station"], "radius": 190})
        prefix = []
        field = min(self.world.fields.values(), key=lambda f: f.altitude(start))
        if field.altitude(start) < 2600:
            normal = start - field.center
            normal.normalize()
            climb = field.center + normal * (field.radius + field.elevation(normal) + 2650)
            prefix.append(climb)
            start = climb
        if destination["kind"] == "planet":
            target = self.system["planets"][destination["index"]]
            field = self.world.fields[target["id"]]
            # Survey navigation returns to the recorded home landing district.
            # Manual pilots may enter and land anywhere on the sphere.
            goal = field.point(11, 2, 2650)
            destination["landing_world"] = field.point(11, 2, 35)
        else:
    ...
    def _update_autopilot(self, dt):
        ...
        if dt <= 0:
            return True
        dest = self.autopilot
        route, index = dest["route"], dest["route_index"]
        point = self.world_position()
        if index >= len(route):
            if dest["kind"] == "planet" and dest["phase"] != "descent":
                dest.update(route=[dest["landing_world"]], route_index=0, phase="descent")
                route, index = dest["route"], 0
            elif dest["kind"] == "planet":
                if self._touch_down():
                    return True
                self.autopilot = None
                return True
                ...
                return True
        local_goal = frame_to_local(self.frame, route[index])
        delta = local_goal - self.controller.position
        remaining = delta.length()
        if remaining < .35:
            dest["route_index"] = index + 1
            return True
        direction = delta / remaining
        blend = self.atmosphere_data["space_blend"]
        ceiling = 150 + 4850 * blend * blend
        if dest["phase"] == "descent":
            ceiling = min(ceiling, 60 + max(0, self.atmosphere_data["altitude"] - 60) * .35)
        desired_speed = min(ceiling, remaining * 1.15)
        # Braking follows the distance to every waypoint, including touchdown.
        previous = Vec3(self.controller.velocity)
        velocity = previous + (direction * desired_speed - previous) * (1 - math.exp(-3 * dt))
        displacement = (previous + velocity) * (.5 * dt)
        if displacement.length() > remaining and displacement.dot(direction) > 0:
            displacement *= remaining / displacement.length()
        result = self.world.move_sphere(self.controller.position, displacement, radius=3)
        ...
        self.controller.throttle = min(1, desired_speed / max(1, ceiling))
        self.camera.setPos(result.position)
        if (expected - result.position).length() > .15:
            self.autopilot = None
            for normal in result.normals:
                inward = self.controller.velocity.dot(normal)
                if inward < 0:
                    self.controller.velocity -= normal * inward
            self.controller.speed = self.controller.velocity.length()
            self.controller.collision_feedback = 1
            self.toast("Approach paused by an obstruction. Steer clear, then use M to plot again.", "alert")
            return True
        # Keep current physical roll, smoothly turn toward the route direction.
        target = self.camera.getQuat()
        marker = NodePath("approach-attitude")
        marker.setPos(0, 0, 0)
        marker.lookAt(direction, self.camera.getQuat().getUp())
        target = marker.getQuat()
        # Quaternion component interpolation follows the short arc.
        current = self.camera.getQuat()
        if current.dot(target) < 0:
            target = -target
        attitude = current * (1 - min(1, dt * 3)) + target * min(1, dt * 3)
        attitude.normalize()
        self.controller.heading, self.controller.pitch, self.controller.reference_roll = map(float, attitude.getHpr())
        self.controller.camera_roll = 0
        self.camera.setQuat(attitude)
    ...
    def step(self, dt):
        ...
        self.audio.set_atmosphere(self.atmosphere_data)
        self.audio.update(dt, self.controller.mode, abs(self.controller.speed) / 100)
        self.atmosphere_effects.update(dt if self.playing else 0, self.atmosphere_data,
                                       self.controller.mode if self.started else "hidden",
                                       self.game.settings.get("camera_motion", .35))
    ...
    def _view(self):
        ...
        return {"mode": mode, "location": self.system["name"] if mode == "orbit" else self.planet["name"],
                ...
                "planet_altitude": self.atmosphere_data["altitude"],
    ...
    def _flight_phase(self):
        data = self.atmosphere_data
        if self.controller.mode == "surface":
            return "SURFACE"
        if data["space_blend"] >= .99999:
            return "INTERPLANETARY"
        if data["heat"] > .08:
            return "ATMOSPHERIC ENTRY"
        if data["radial_speed"] > 3:
            return "ATMOSPHERIC ASCENT"
        if data["radial_speed"] < -3:
            return "ATMOSPHERIC DESCENT"
        ...
        if kind == "inventory":
            ...
            if kind == "galaxy":
                ...
                if self.controller.mode == "surface":
                    ...
                    rows.append({"title": "Your courier ship", "body": "E or F boards and launches. Fly upward through the clouds and atmosphere to reach space.",
                ...
                rows.append({"title": "Orbital Exchange", "body": "Trade materials, restore your suit, and accept survey contracts.",
                             "meta": "Automatic approach available while flying",
                             "buttons": [self.button("APPROACH STATION", "approach_station", enabled=self.controller.mode != "surface")]})
    ...
    def action(self, action, payload=None):
        ...
        if action == "close":
            ...
            if any(distance(self._geo_to_local(record["pos"]), pos) < 9 for record in records):
            ...
            if not self._site_clear(pos.x, pos.y, radius=clearance, height=5):
                ...
                return
            geo = self._geo_position(pos)
            geographic_frame = PlanetFrame.from_planet(self.planet, chart_direction(geo[0], geo[1], self.planet["size"]))
            forward = geographic_frame.vector_to_local(vector_to_world(self.frame, self.controller.forward()))
            heading = math.degrees(math.atan2(-forward.x, forward.y))
            ok, message, record = self.game.build(payload, self.planet["id"], geo, heading)
            if ok:
    ...
    def run_smoke_checks(self):
        ...
        for _ in range(3000):
            self.step(.1)
            if self.controller.mode == "surface" or not self.autopilot:
                break
        check("continuous planetary approach lands and records a visit", self.controller.mode == "surface" and len(self.game.visited) == 2)
        ...
        return {"game": "Asterion Expedition", "version": __version__, "status": "passed", "checks": checks,
                "tested": sys.platform + ", Python " + sys.version.split()[0] + ", offscreen rendering", "save_directory": str(self.save_dir)}
