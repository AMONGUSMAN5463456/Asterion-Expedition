...
from asterion.planetary import frame_to_world, vector_to_world
...
class NativeIntegrationTests(unittest.TestCase):
    ...
    def advance_until(self, predicate, frames=1200):
        for _ in range(frames):
            if predicate():
                return
            self.app.step(.1)
        self.fail(f"Gameplay did not reach the expected state within {frames * .1:g} simulated seconds; "
                  f"mode={self.app.controller.mode}, autopilot={self.app.autopilot}, "
                  f"notice={self.app.notice!r}")
    ...
    def test_construction_materials_and_world_nodes_survive_travel_and_reload(self):
        ...
        building = next(e for e in self.app.world.interactables() if e["id"] == record["id"])
        building_world = Vec3(building["node"].getPos(self.app.render))
        building_world = frame_to_world(self.app.frame, building_world)
        solar_root = self.app.world.root
        self.app.enter_orbit()
        self.assertEqual(self.app.world.root, solar_root)
        self.assertFalse(building["node"].isEmpty())
        self.assertLess(distance(building["node"].getPos(self.app.render), building_world), .02)
        # Explicit fixture placement exercises geographic unload/return without
        # asking the navigation command to teleport between planets.
        self.app.game.planet_index = 1
        self.app._load_surface(fresh=True)
        self.assertNotIn(record["id"], {e["id"] for e in self.app.world.interactables()})
        self.app.game.planet_index = 0
        self.app._load_surface(fresh=True)
    ...
    def test_constructed_habitat_actually_shelters_the_player(self):
        ...
        x, y, _ = self.app._geo_to_local(record["pos"])
        self.app.controller.set_mode("surface", (x, y - 5, self.app.world.height(x, y - 5) + 1.8))
    ...
    def test_surface_flight_save_restores_flight_pose_and_velocity_without_parked_ship(self):
        ...
        self.app.controller.set_mode("flight", (154, -89, 300), 126, -15)
        self.app.controller.velocity = Vec3(12, 35, -8)
        self.app.controller.reference_roll = 24
        self.app.camera.setHpr(126, -15, 24)
        position = self.app.world_position()
        velocity = vector_to_world(self.app.frame, self.app.controller.velocity)
        forward = vector_to_world(self.app.frame, self.app.camera.getQuat().getForward())
        up = vector_to_world(self.app.frame, self.app.camera.getQuat().getUp())
        fuel = self.app.game.vitals["fuel"]
        self.assertTrue(self.app.save_game(False))
        self.app.continue_game()
        self.assertEqual(self.app.controller.mode, "flight")
        self.assertLess(distance(self.app.world_position(), position), .02)
        self.assertLess(distance(vector_to_world(self.app.frame, self.app.controller.velocity), velocity), .001)
        self.assertLess(distance(vector_to_world(self.app.frame, self.app.camera.getQuat().getForward()), forward), .001)
        self.assertLess(distance(vector_to_world(self.app.frame, self.app.camera.getQuat().getUp()), up), .001)
        self.assertIsNone(self.app.ship)
        self.assertEqual(self.app.game.vitals["fuel"], fuel)
        self.assertEqual(self.app.controller.reference_roll, 24)

    def test_old_altitude_boundary_keeps_world_and_low_touchdown_parks_one_ship(self):
        self.app.recall_ship()
        self.app.launch()
        ...
        self.assertTrue(self.app.fade.isHidden())
        self.assertEqual(self.app.controller.mode, "flight")
        self.assertEqual(self.app.world.root, previous)
        self.assertFalse(previous.isEmpty())
        self.assertIsNone(self.app.ship)
        position = self.app.world_position()
        self.app.flight_action()
        self.assertEqual(self.app.controller.mode, "flight")
        self.assertEqual(self.app.world_position(), position)
        self.assertIsNone(self.app.transition)
        self.assertIsNone(self.app.ship)
        # F is a local low-speed touchdown action at actual terrain clearance.
        self.app.controller.set_mode("flight", (p.x, p.y, self.app.world.height(p.x, p.y) + 30))
        self.app._update_flight_environment()
        self.app.flight_action()
        self.assertEqual(self.app.controller.mode, "surface")
        self.assertEqual(self.app.world.root, previous)
        self.assertEqual(self.app.render.findAllMatches("**/persistent-solar-system").getNumPaths(), 1)
        self.assertEqual(self.app.render.findAllMatches("**/wayfarer-survey-craft").getNumPaths(), 1)
    ...
    def test_station_approach_pauses_cancels_and_opens_a_real_market(self):
        ...
        self.assertAlmostEqual(distance(self.app.world_position(), self.app.system["station"]), 320, delta=.5)
    ...
    def test_failed_parking_keeps_landing_local_and_cannot_launch_absent_ship(self):
        ...
        landing_world = frame_to_world(self.app.frame, (400, 500, self.app.world.height(400, 500)))
        with patch.object(self.app, "_park_ship", return_value=False):
            self.app._load_surface(landing=(400, 500))
        self.assertLess(distance(self.app.world_position(), landing_world), 35)
        self.assertIsNone(self.app.ship)
        fuel = self.app.game.vitals["fuel"]
        self.app.launch()
        self.assertEqual(self.app.controller.mode, "surface")
